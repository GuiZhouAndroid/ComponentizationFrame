package zsdev.work.lib.support.dialog.custom.loadingdrawable.animation;
...
import zsdev.work.lib.support.dialog.custom.loadingdrawable.DensityUtil;
import zsdev.work.lib.support.dialog.custom.loadingdrawable.LoadingRenderer;
...
public class GhostsEyeLoadingRenderer extends LoadingRenderer {
    ...
    private final Interpolator EYE_BALL_INTERPOLATOR = new EyeBallInterpolator();
    private final Interpolator EYE_CIRCLE_INTERPOLATOR = new EyeCircleInterpolator();
    ...
    private float mMaxEyeJumpDistance;
    ...
    private void init(Context context) {
        ...
        mMaxEyeJumpDistance = DensityUtil.dip2px(context, DEFAULT_MAX_EYE_JUMP_DISTANCE);
    ...
    protected void computeRender(float renderProgress) {
        if (renderProgress <= LEFT_EYE_BALL_END_JUMP_OFFSET && renderProgress >= LEFT_EYE_CIRCLE$BALL_START_JUMP_UP_OFFSET) {
            ...
            mLeftEyeBallOffsetY = -mMaxEyeJumpDistance * EYE_BALL_INTERPOLATOR.getInterpolation(eyeCircle$BallJumpUpProgress);
        }
        ...
        if (renderProgress <= LEFT_EYE_CIRCLE_END_JUMP_OFFSET && renderProgress >= LEFT_EYE_CIRCLE$BALL_START_JUMP_UP_OFFSET) {
            ...
            mLeftEyeCircleOffsetY = -mMaxEyeJumpDistance * EYE_CIRCLE_INTERPOLATOR.getInterpolation(eyeCircle$BallJumpUpProgress);
        }
        ...
        if (renderProgress <= RIGHT_EYE_BALL_END_JUMP_OFFSET && renderProgress >= RIGHT_EYE_CIRCLE$BALL_START_JUMP_UP_OFFSET) {
            ...
            mRightEyeBallOffsetY = -mMaxEyeJumpDistance * EYE_BALL_INTERPOLATOR.getInterpolation(eyeCircle$BallJumpUpProgress);
        }
        ...
        if (renderProgress <= RIGHT_EYE_CIRCLE_END_JUMP_OFFSET && renderProgress >= RIGHT_EYE_CIRCLE$BALL_START_JUMP_UP_OFFSET) {
            ...
            mRightEyeCircleOffsetY = -mMaxEyeJumpDistance * EYE_CIRCLE_INTERPOLATOR.getInterpolation(eyeCircle$BallJumpUpProgress);
        }
    }
    ...
    private RectF createLeftEyeBall(RectF arcBounds, float offsetY) {
        ...
        return new RectF(leftEyeCenterX - mEyeBallWidth / 2.0f, leftEyeCenterY - mEyeBallHeight / 2.0f,
    ...
    private RectF createRightEyeBall(RectF arcBounds, float offsetY) {
        ...
        return new RectF(rightEyeCenterX - mEyeBallWidth / 2.0f, rightEyeCenterY - mEyeBallHeight / 2.0f,
    ...
    private static class EyeCircleInterpolator implements Interpolator {
    ...
    private static class EyeBallInterpolator implements Interpolator {
